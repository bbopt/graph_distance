...
if __name__ == '__main__':
        ...
        for j in range(nb_instances_per_variant):
            ...
            for approach in approaches:
                for model in models:
                    ...
                    data = pd.read_csv("logs_test/"+data_file, delim_whitespace=True, header=None)
                    evals = data[0].tolist()
    ...
    plt.rc('legend', fontsize=plt_size/1.25)
    ...
    #fig.suptitle("Data profiles on RMSE test", fontsize=plt_size)
    #fig.suptitle("Test", fontsize=plt_size)
    ...
    for idx_tau, tau in enumerate(taus):
        ...
        if idx_tau == 1:
            ax[idx_tau].set_xlabel(r'Groups of $(n_{p,s}+1)$ evaluations', fontsize=plt_size)
