...
if __name__ == '__main__':
    ...
    if load_RMSE_dataset:
        ...
        #RMSE_naive = np.zeros((nb_pts_train-3+1, nb_rand_trials))
        #RMSE_graph = np.zeros((nb_pts_train-3+1, nb_rand_trials))
        #RMSE_hybrid = np.zeros((nb_pts_train-3+1, nb_rand_trials))
        ...
        for j in range(nb_rand_trials):
            ...
            while len(draw) != 0:
                ...
                # Create partial data set for hybrid approach : variant 1, it's the same as graph
                ...
                if model == "IDW":
                    ...
                    IDW_naive_bb_test = variant1_bb_wrapper(X_train_naive_partial, y_train_naive_partial,
                    ...
                    IDW_graph_bb_test = variant1_bb_wrapper(X_train_graph_partial, y_train_graph_partial,
                                                             X_test, y_test, nb_max_var, nb_var_sub, "graph", "IDW")
                    RMSE_graph[i, j] = IDW_graph_bb_test(*x_best_graph_IDW)

                    # For variant 1, hybrid has the same training and test sets, hence we can reuse X_test and y_test
                    IDW_hybrid_bb_test = variant1_bb_wrapper(X_train_hybrid_partial, y_train_hybrid_partial,
